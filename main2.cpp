...
void inputPoints(vector<PolygonPoint*> &points)
{
...
void initPoints(vector<PolygonPoint*> &points)
{
  int numPoints = points.size();
  for(int i=0;i<numPoints;i++)
  {
...
void monotonePartition(map<PolygonPoint, Line> &inverseHelper, vector<PolygonPoint*> &points)
{
  set<Line> setLines;
...
int main()
{
  ...
  monotonePartition(inverseHelper, points);

  t(points[0]->getNextLine().to_string());

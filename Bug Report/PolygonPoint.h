...
typedef struct PolygonPoint
{
  ...
  void setNextPoint(PolygonPoint &next)
  {
    // nextLine = Line(*this,next);
    nextPoint = new PolygonPoint();
    *nextPoint = next;
  }

  void setPrevPoint(PolygonPoint &prev)
  {
    // prevLine = Line(prev,*this);
    prevPoint = new PolygonPoint();
    *prevPoint = prev;
  }
